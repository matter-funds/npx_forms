...
data_dir:pwd,"/../../data/";
parsed_npx_paths:system"ls ",data_dir,"/npx_forms_parsed/STATE_STREET_INSTITUTIONAL_FUNDS/*/*500_Index*";
...
ps:raze ps;
ps:select from ps where not VoteId like "*_18"; /incomplete data for 2018 report.

/drop bogus entries (badly parsed for now)
ps:select from ps where not null Num, not null NumType;
ps:update MeetDate_adj:MeetDate - 191 from ps;
